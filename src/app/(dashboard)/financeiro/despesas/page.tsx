...
export default async function DespesasPage({ searchParams }: Props) {
  ...
  assertAdministrativeRole(session.user.role, 'Sem permissao para consultar as despesas da barbearia.')
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3 text-sm font-medium text-foreground">
                          {expense.description}
                          {expense.recurrent && (
                          <span className="surface-chip ml-2 rounded px-1.5 py-0.5 text-[10px] font-medium">
                            recorrente
                          </span>
                          )}
                        </td>
        ...
        <aside className="premium-rail p-6">
          ...
          <div className="mt-5 space-y-3">
            ...
            <div className="tonal-note">
              ...
              <div className="mt-3 space-y-2">
                {categories.slice(0, 3).map((category) => (
                  <div key={category.id} className="flex items-center justify-between gap-3 rounded-xl bg-background/30 px-3 py-2.5">
                    <div>
            ...
            <div className="flex flex-wrap gap-2">
              <Link href="/financeiro/categorias" className="action-button-primary">
                <TrendingDown className="h-4 w-4" />
                Organizar categorias
              </Link>
              <Link href="/financeiro/fluxo-caixa" className="action-button">
                <ArrowUpRight className="h-4 w-4" />
                Ver fluxo de caixa
              </Link>
            </div>
